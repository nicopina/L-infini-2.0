...
function SalesOfSelectedDay(params) {
  ...
  const [countOrdersDay, setCountOrdersDay] = React.useState(
    params.countOrdersDay
  );

  useEffect(() => {
    const fecha_ms = fecha.getTime();

    getProfitByOneDate(fecha_ms + fecha.getTimezoneOffset() * 60000).then(
      (response) => {
    ...
    getCountOrdersOneFullDate(
      fecha_ms + fecha.getTimezoneOffset() * 60000
    ).then((response) => {
  ...
  function formatear_dinero(dinero) {
    ...
    var dinero_formateado = parseInt(dinero)
      .toString()
      .replace(/\B(?=(\d{3})+(?!\d))/g, ".");
  ...
  function calcular_formatear_promedio(dinero, cantidad) {
    ...
    var promedio_formateado = parseInt(promedio)
      .toString()
      .replace(/\B(?=(\d{3})+(?!\d))/g, ".");
      ...
      <Card className="card-chart">
        ...
        <CardBody>
          <h4 style={{ textAlign: "center", color: "black" }}>
            Total de ventas del día seleccionado: ${formatear_dinero(dayProfit)}
          </h4>
          <h4 style={{ textAlign: "center", color: "black" }}>
            Total de pedidos del día seleccionado:{" "}
            {formatear_dinero(countOrdersDay)}
          </h4>
          <h4 style={{ textAlign: "center", color: "black" }}>
            Promedio de ingresos por pedido: $
            {calcular_formatear_promedio(dayProfit, countOrdersDay)}
          </h4>
        </CardBody>
      </Card>
    </div>
