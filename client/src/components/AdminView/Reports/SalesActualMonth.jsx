...
function SalesActualMonth(params) {
  const [profitEntireMonth, setProfitEntireMonth] = useState(
    params.profitEntireMonth
  );
  const [countOrdersMonth, setCountOrdersMonth] = useState(
    params.countOrdersMonth
  );
  ...
  var formatedProfit = profitEntireMonth
    ? profitEntireMonth.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ".")
    : 0;
  var formatedCountOrdersMonth = countOrdersMonth
    ? countOrdersMonth.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ".")
    : 0;
  var formatedAverageProfit = countOrdersMonth
    ? parseInt(profitEntireMonth / countOrdersMonth)
        .toString()
        .replace(/\B(?=(\d{3})+(?!\d))/g, ".")
    : 0;
        ...
        <CardBody style={{ textAlign: "center" }}>
          <ListGroup>
            <ListGroupItem>
              ...
              <h4>
                Total de pedidos realizados en el mes actual:{" "}
                {formatedCountOrdersMonth}
              </h4>
              <h4>
                Promedio de ventas por pedido en el mes actual: $
                {formatedAverageProfit}
              </h4>
            </ListGroupItem>
            <ListGroupItem>
